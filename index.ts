...
export default class DBLog implements DBProps {
  ...
  async continue(title: string = this.options.title || "n/a", msg: string = this.options.msg || "n/a", props?: PlainObj): Promise<any | null> {
    ...
    return this.start(title, msg, props);
  }

  async start(title: string = this.options.title || "", msg: string = this.options.msg || "", props?: PlainObj) {
    log && log(msg, props);
  ...
  async updateAll(props: PlainObj) {
    if (!props) { return; } // null id means id has been cleared due to error
  ...
  delete = {
    ...
    previous: async (title: string = this.options.title || "") => {
      ...
      delete
      from dbo.log 
      where title = 'rebuild-search-index' 
      and logid not in (
